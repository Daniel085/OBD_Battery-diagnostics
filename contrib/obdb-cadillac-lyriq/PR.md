...
| --- | --- | --- | --- |
| `LYRIQ_HVBAT_A` **(pack current)** | DA17 · `222414` | s16 ÷ 20 → A, − = charging | −22.75 A steady at a 9.22 kW AC charge; +0.9 A aux draw in Ready; live jitter. Bolt `7E1.222414` crossover. |
| `LYRIQ_HVBAT_NOMINAL_V` | DA17 · `222429` | u16 ÷ 64 → V | Constant 352.1 V at 70%-charging AND 80%-Ready → nominal rating, not live. Copies at `2428/242D/2434/2489`. |
| `LYRIQ_ODOMETER` | DA40 · `22448F` | u32 ÷ 64 → km | 2,031,186/64 = 31,737 km = 19,720 mi — matched the dash exactly. (`22415B` is the same register.) |
| `LYRIQ_EVSE_PILOT_A` | DA40 · `224149` | u16 ÷ 10 → A | 38.8 A plugged into a 9.2 kW EVSE; snaps to 10.0 A default on unplug. |
| `LYRIQ_HVBAT_GRP1_V`..`GRP3_V` | DA40 · `22416C/D/E` | u16 ÷ 100 → V | ~51 V groups; move with charge/load; read 0 when vehicle off (live, behind contactors). |
| `LYRIQ_HVBAT_TEMP` | DA40 · `22434F` | byte − 40 → °C | Matches the ÷32 temp cluster. |
| `LYRIQ_HVBAT_TEMP_A/B` | DA40 · `224127/224124` | u16 ÷ 32 → °C | Pack temp sensors. |
| `LYRIQ_HVBAT_COOLANT_T1/T2` | DA40 · `2240E5/2240E6` | u16 ÷ 32 → °C | Rose under charge (coolant loop). |
| `LYRIQ_DMCM_BATT_V` | DA1D · `2233E5` | byte ÷ 10 → V | See correction below. |
...
This PR ships it as `LYRIQ_DMCM_BATT_V` ("DMCM battery voltage"), matching the naming already used by `OBDb/Chevrolet-Equinox-EV` for the same DID on the same platform — which independently corroborates the correction.
